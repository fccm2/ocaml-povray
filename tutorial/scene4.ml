...
let () =
  ...
  let ms =
    List.init 5 (fun _ ->
      let x = rand_num (-1.6, 1.0) in
      let z = rand_num (-1.6, 1.0) in
      let a = rand_num (0.0, 90.0) in
      ((x, 0.0, z), (0.0, a, 0.0))
    )
  in
  (* will produce something like:
    [
      (( 1.7, 0.0, -0.3), (0., 108.2, 0.));
      (( 1.4, 0.0,  1.2), (0., 113.1, 0.));
      ((-1.3, 0.0, -0.4), (0., 101.3, 0.));
      ((-0.9, 0.0, -1.6), (0., 136.8, 0.));
      ((-0.7, 0.0,  0.7), (0., 178.6, 0.));
    ]
  *)
